...
sp_rename '{1}', '{2}', 'COLUMN'";
        ...
        public void export(Command cmd, Configuration cfg, ShellContext context)
        {
            if (cmd.HasHelp)
            {
                ...
                stdio.WriteLine("      [/ns:name] default name space is defined on the .cfg");
                stdio.WriteLine("      [/class:name] default class name is defined on the .cfg");
                stdio.WriteLine("      [/type:const|default|field|property|hierarchy] class type, default is const");
                stdio.WriteLine("             const: generate const key");
                stdio.WriteLine("           default: generate default value");
                stdio.WriteLine("          property: generate static property to get setting");
                stdio.WriteLine("             field: generate static field to get setting");
                stdio.WriteLine("         hierarchy: generate hierarchial keys to get setting");
